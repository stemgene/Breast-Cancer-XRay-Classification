...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    print(f"Train F1 Score: {f1:.3f}, Recall: {recall:.3f}, Precision: {precision:.3f}")
    ...
    return accu_loss.item() / (step + 1), accu_num.item() / sample_num, f1, recall, precision, cm
...
def evaluate(model, data_loader, device, epoch):
    ...
    recall = recall_score(all_labels, all_preds, average='macro')
    precision = precision_score(all_labels, all_preds, average='macro')
    print(f"F1 Score: {f1:.3f}, Recall: {recall:.3f}, Precision: {precision:.3f}")
